...
interface MerchantRecord {
  ...
  hasEbayDeal: boolean | null;
  amazonDeepLink?: string | null;
  lastAttempt?: string;
  processedAt?: string;
}
...
async function extractDomainsFromScrapeState(
  ...
  try {
    ...
    // Extract storeDomain from each merchant record
    for (const record of data.merchantRecords) {
      if (record.storeDomain) {
        domains.add(record.storeDomain);
      }
    }
...
function generateComparisonRecords(
  ...
  return scrapeState.merchantRecords
    .filter((record) => record.storeDomain)
    .map((record) => {
      const domain = record.storeDomain!;
      ...
      const hasContextualAd =
        record.amazonDeepLink?.includes("https://www.amazon.com/s?k") ?? false;
      ...
      return {
        ...
        "Has Contextual Ad": hasContextualAd,
...
async function exportComparisonData(
  ...
  const csvWriter = createObjectCsvWriter({
    ...
    header: [
      ...
      { id: "Has Contextual Ad", title: "Has Contextual Ad" },
  ...
  const combinedJson = {
    summary: {
      ...
      refermateDomainsCount: scrapeState.merchantRecords.length,
...
async function main() {
  try {
    ...
    // Extract domains from scrape-state.json
    const scrapeStateFilePath = path.join(
      "output",
      "refermate-scrape-state.json"
    );
    ...
    competitorMap.set("Refermate", scrapeStateDomains);

    // Compare DealsPotr against scrape-state domains
    console.log(
      "\nCalculating domain overlap between DealsPotr and Refermate domains..."
    );
    ...
    const uniqueInRefermate = new Set<string>();
    for (const domain of scrapeStateDomains) {
      if (!dealsptrDomains.has(domain)) {
        uniqueInRefermate.add(domain);
      }
    }

    console.log(`\nUnique domains in DealsPotr: ${uniqueInDealspotr.size}`);
    console.log(`Unique domains in Refermate: ${uniqueInRefermate.size}`);
    ...
    await exportComparisonData(
      ...
      dealsptrDomains,
      dealsptrUrlMap,
      path.join(csvOutputDir, "refermate.com-comparison.csv"),
      path.join(outputDir, "refermate.com-comparison.json"),
      competitorResults,
      affiliateData
    );

    console.log(
      `\nResults saved to ${outputDir}/refermate.com-comparison.json`
    );
